  Dropzone.options.opportunityFilesUpload = false;
  Dropzone.options.opportunityExpenseForm = false;

  var expenseDropzone;
  $(function() {

      $('.opportunity-tabs-and-opts-toggler').on('click', function(e) {
          e.preventDefault();
          slideToggle('.opportunity-menu-panel');
          slideToggle('.opportunity-toggler-opts');
      });

      init_ajax_search('customer', '#clientid_copy_opportunity.ajax-search');

      // remove the divider for opportunity actions in case there is no other li except for pin opportunity
      $('ul.opportunity-actions li:first-child').next('li.divider').remove();

      var file_id = get_url_param('file_id');
      if (file_id) {
          view_opportunity_file(file_id, opportunity_id);
      }

      // Fix for shortcuts in discussions textarea/contenteditable - jquery-comments plugin
      var $discussionsContentEditable = $('#opportunity_file_data,#discussion-comments');
      ...
      $('body').on('show.bs.modal', '._opportunity_file', function() {
          discussion_comments('#opportunity-file-discussion', discussion_id, 'file');
      });
      ...
      initDataTable('.table-credit-notes', admin_url + 'credit_notes/table?opportunity_id=' + opportunity_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);

      if ($('#timesheetsChart').length > 0 && typeof(opportunity_overview_chart) != 'undefined') {
          var chartOptions = {
              type: 'bar',
              data: {},
              options: {
          ...
          chartOptions.data = opportunity_overview_chart.data;
          var ctx = document.getElementById("timesheetsChart");
          timesheetsChart = new Chart(ctx, chartOptions);
      }
      milestones_kanban();
      $('#opportunity_top').on('change', function() {
          var val = $(this).val();
          var __opportunity_group = get_url_param('group');
          if (__opportunity_group) {
              __opportunity_group = '?group=' + __opportunity_group;
          } else {
              __opportunity_group = '';
          }
          window.location.href = admin_url + 'opportunities/view/' + val;
      });

      if (typeof(Dropbox) != 'undefined' && $('#dropbox-chooser').length > 0) {
          document.getElementById("dropbox-chooser").appendChild(Dropbox.createChooseButton({
              success: function(files) {
                  $.post(admin_url + 'opportunities/add_external_file', {
                      files: files,
                      ...
                      window.location.href = location.split('?')[0] + '?group=opportunity_files';
                  });
              },
              linkType: "preview",
      ...
      $('body').on('click', '.milestone-column .cpicker,.milestone-column .reset_milestone_color', function(e) {
          ...
          $.post(admin_url + 'opportunities/change_milestone_color', {
              color: color,
      ...
      if ($('#opportunity-files-upload').length > 0) {
          new Dropzone('#opportunity-files-upload', $.extend({}, _dropzone_defaults(), {
              paramName: "file",
              uploadMultiple: true,
              parallelUploads: 20,
              maxFiles: 20,
              ...
              success: function(file, response) {
                  if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                      window.location.href = admin_url + 'opportunities/view/' + opportunity_id + '?group=opportunity_files';
                  }
              },
      ...
      if ($('#opportunity-expense-form').length > 0) {
          expenseDropzone = new Dropzone("#opportunity-expense-form", $.extend({}, _dropzone_defaults(), {
      ...
      _validate_form($('#opportunity-expense-form'), {
          category: 'required',
          date: 'required',
          amount: 'required',
          currency: 'required'
      }, opportunityExpenseSubmitHandler);
      ...
      gantt = $("#gantt").gantt({
          ...
          onRender: function() {
              ...
              $('#gantt .leftPanel .spacer').html('<span class="gantt_opportunity_name"><i class="fa fa-cubes"></i> ' + $('.opportunity-name').text() + '</span>');
              var _percent = $('input[name="opportunity_percent"]').val();
          ...
          onAddClick: function(dt, rowId) {
              ...
              new_task(admin_url + 'tasks/task?rel_type=opportunity&rel_id=' + opportunity_id + '&start_date=' + d1);
          }
      });
      ...
      _table_api = initDataTable('.table-opportunity-expenses', admin_url + 'opportunities/expenses/' + opportunity_id, 'undefined', 'undefined', Expenses_ServerParams, [4, 'desc']);

      if (_table_api) {
          _table_api.column(0).visible(false, false).columns.adjust();
      }

      init_rel_tasks_table(opportunity_id, 'opportunity');
      initDataTable('.table-notes', admin_url + 'opportunities/notes/' + opportunity_id, [4], [4], 'undefined', [1, 'desc']);
      ...
      initDataTable('.table-timesheets', admin_url + 'opportunities/timesheets/' + opportunity_id, [8], [8], Timesheets_ServerParams, [3, 'desc']);
      initDataTable('.table-opportunity-discussions', admin_url + 'opportunities/discussions/' + opportunity_id, undefined, undefined, 'undefined', [1, 'desc']);
      ...
      $('#timesheet select[name="timesheet_task_id"]').on('change', function() {
          ...
          requestGet('opportunities/timesheet_task_assignees/' + _task_id + '/' + opportunity_id + '/' + staff_id).done(function(response) {
              select_staff.html(response);
              select_staff.selectpicker('refresh');
          });
      });

      $('body').on('change', '#opportunity_invoice_select_all_tasks,#opportunity_invoice_select_all_expenses', function() {
  ...
  function milestones_switch_view() {
      ...
      $('.opportunity-milestones-kanban').toggleClass('hide');
      if (!$.fn.DataTable.isDataTable('.table-milestones')) {
          initDataTable('.table-milestones', admin_url + 'opportunities/milestones/' + opportunity_id);
      }
  }
  ...
  function manage_discussion(form) {
      ...
      $.post(url, data).done(function(response) {
          ...
          $('.table-opportunity-discussions').DataTable().ajax.reload(null, false);
  ...
  function mass_stop_timers(only_billable) {
      requestGetJSON('opportunities/mass_stop_timers/' + opportunity_id + '/' + only_billable).done(function(response) {
          alert_float(response.type, response.message);
          setTimeout(function() {
              $('body').find('.modal-backdrop').eq(0).remove();
              init_timers();
              reload_tasks_tables();
              pre_invoice_opportunity();
          }, 500);
      });
  }

  function pre_invoice_opportunity() {
      requestGet('opportunities/get_pre_invoice_opportunity_info/' + opportunity_id).done(function(response) {
          $('#pre_invoice_opportunity').html(response);
          $('#pre_invoice_opportunity_settings').modal('show');
      });
  }

  function invoice_opportunity(opportunity_id) {
      $('#pre_invoice_opportunity_settings').modal('hide');
      var data = {};
      ...
      $.post(admin_url + 'opportunities/get_invoice_opportunity_data/', data).done(function(response) {
          $('#invoice_opportunity').html(response);
          $('#invoice-opportunity-modal').modal({
              show: true,
              backdrop: 'static'
          });
      });
  ...
  function delete_opportunity_discussion(id) {
      if (confirm_delete()) {
          requestGetJSON('opportunities/delete_discussion/' + id).done(function(response) {
  ...
  function opportunityExpenseSubmitHandler(form) {
  ...
  function view_opportunity_file(id, $opportunity_id) {
      $('#opportunity_file_data').empty();
      $("#opportunity_file_data").load(admin_url + 'opportunities/file/' + id + '/' + opportunity_id, function(response, status, xhr) {
          if (status == "error") {
  ...
  function update_file_data(id) {
      ...
      $.post(admin_url + 'opportunities/update_file_data/', data);
  }

  function opportunity_mark_as_modal(status_id, $opportunity_id) {
      $('#mark_tasks_finished_modal').modal('show');
      $('#opportunity_mark_status_confirm').attr('data-status-id', status_id);
      $('#opportunity_mark_status_confirm').attr('data-opportunity-id', opportunity_id);
      var $opportunityMarkedasFinishedInput = $('#opportunity_marked_as_finished_email_to_contacts');
      if (status_id == 4) {
          if ($opportunityMarkedasFinishedInput.length > 0) {
              $opportunityMarkedasFinishedInput.parents('.opportunity_marked_as_finished').removeClass('hide');
          }
      } else {
          if ($opportunityMarkedasFinishedInput.length > 0) {
              $opportunityMarkedasFinishedInput.prop('checked', false);
              $opportunityMarkedasFinishedInput.parents('.opportunity_marked_as_finished').addClass('hide');
          }
      }
  }

  function opportunity_files_bulk_action(e) {
      if (confirm_delete()) {
          ...
          var rows = $('.table-opportunity-files').find('tbody tr');
          $.each(rows, function() {
          ...
          setTimeout(function() {
              $.post(admin_url + 'opportunities/bulk_action_files', data).done(function() {
                  window.location.reload();
              });
  ...
  function gantt_filter() {
      ...
      params['group'] = 'opportunity_gantt';
      if (status) {
          params['gantt_task_status'] = status;
      }
      window.location.href = buildUrl(admin_url + 'opportunities/view/' + opportunity_id, params);
  }

  function confirm_opportunity_status_change(e) {
      var data = {};
      $(e).attr('disabled', true);
      data.opportunity_id = $(e).data('opportunity-id');
      data.status_id = $(e).data('status-id');
      if (data.status_id == 4) {
          var $opportunityMarkedasFinishedInput = $('#opportunity_marked_as_finished_email_to_contacts');
          if ($opportunityMarkedasFinishedInput.length > 0) {
              data.send_opportunity_marked_as_finished_email_to_contacts = $opportunityMarkedasFinishedInput.prop('checked') === true ? 1 : 0;
          }
      }
      data.mark_all_tasks_as_completed = $('#mark_all_tasks_as_completed').prop('checked') === true ? 1 : 0;
      data.notify_opportunity_members_status_change = $('#notify_opportunity_members_status_change').prop('checked') === true ? 1 : 0;
      $.post(admin_url + 'opportunities/mark_as', data).done(function(response) {
          response = JSON.parse(response);
  ...
  function milestones_kanban_update(ui, object) {
      if (object === ui.item.parent()[0]) {
          ...
          setTimeout(function() {
              $.post(admin_url + 'opportunities/update_task_milestone', data)
          }, 50);
      }
  }

  function milestones_kanban() {
      init_kanban('opportunities/milestones_kanban', milestones_kanban_update, '.opportunity-milestone', 320, 360, after_milestones_kanban);
  }
  ...
  function after_milestones_kanban() {
      $("#kan-ban").sortable({
          ...
          update: function(event, ui) {
              ...
              $.post(admin_url + 'opportunities/update_milestones_order', data);
          }
      });
  ...
  // When marking task as complete if the staff in on opportunity milestones area, remove this task from milestone in case exists
  function _maybe_remove_task_from_opportunity_milestone(task_id) {
      var $milestonesTasksWrappers = $('.milestone-column');
      if ($("body").hasClass('opportunity') && $milestonesTasksWrappers.length > 0) {
