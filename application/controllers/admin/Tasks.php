...
class Tasks extends Admin_controller
{
    ...
    public function task($id = '')
    {
        ...
        if ($id == '') {
            ...
            if ($data['task']->rel_type == 'opportunity') {
                $data['milestones'] = $this->opportunities_model->get_milestones($data['task']->rel_id);
            }
        ...
        if ($this->input->get('rel_type') == 'opportunity' && $this->input->get('rel_id')) {
            $opportunity = $this->opportunities_model->get($this->input->get('rel_id'));
            if ($project->deadline) {
                $data['project_end_date_attrs'] = [
                    'data-date-end-date' => $opportunity->deadline,
                ];
            }
        }
        $data['id'] = $id;
