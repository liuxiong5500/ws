...
class Clients extends Admin_controller
{
    ...
    {
        parent::__construct();
        $this->not_importable_clients_fields = do_action('not_importable_clients_fields', ['userid', 'id', 'is_primary', 'password', 'datecreated', 'last_ip', 'last_login', 'last_password_change', 'active', 'new_pass_key', 'new_pass_key_requested', 'leadid', 'default_currency', 'profile_image', 'default_language', 'direction', 'show_primary_contact', 'invoice_emails', 'estimate_emails', 'project_emails', 'opportunity_emails', 'task_emails', 'contract_emails', 'credit_note_emails', 'ticket_emails', 'addedfrom', 'registration_confirmed', 'last_active_time']);
    ...
    public function warehouse($customer_id, $warehouse_id = '')
    {
        ...
        if ($this->input->post()) {
            $data = $this->input->post();
            if ($warehouse_id == '') {
                $id = $this->clients_model->add_warehouse($data, $customer_id);
                $message = '';
                $success = false;
                if ($id) {
                    $success = true;
                ...
                $message = '';
                $success = $this->clients_model->update_warehouse($data, $warehouse_id);
                if ($success) {
        ...
        if ($warehouse_id == '') {
            $title = _l('add_new', _l('warehouse_lowercase'));
        } else {
            $data['warehouse'] = $this->clients_model->get_warehouse($warehouse_id);
        ...
        $data['title'] = $title;
        $this->load->view('admin/clients/modals/warehouse', $data);
    }
