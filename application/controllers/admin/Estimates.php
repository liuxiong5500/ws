...
class Estimates extends Admin_controller
{
    public function __construct()
    {
        ...
        $this->load->model('approvedlogs_model');
    }
    ...
    public function estimate($id = '')
    {
        if ($this->input->post()) {
            ...
            $level = $this->levels_model->get_staff_level(get_staff_user_id());
            if ($level) {
                $deparmentid = $level['departmentid'];
                $currentlevel = $level['level'];
            } else {
                $deparmentid = 0;
                $currentlevel = 0;
            }
            $approved = $this->levels_model->get_approved_level($deparmentid, $estimate_data['total'], $estimate_data['lcyprofit']);
            if ($approved) {
                $approvedlevel = $approved['level'];
            } else {
                $approvedlevel = 0;
            }
            $estimate_data['departmentid'] = $deparmentid;
            $estimate_data['approvedlevel'] = $approvedlevel;
            $estimate_data['currentlevel'] = $currentlevel;
    ...
    {
        $this->estimates_model->send_estimate_to_client($id, '', true, '', true);

        $level = $this->levels_model->get_staff_level(get_staff_user_id());
        $this->approvedlogs_model->add([
            'estimateid' => $id,
            'staffid' => get_staff_user_id(),
            'level' => $level['level'],
            'approvedate' => date("Y-m-d H:i:s", time()),
            'status' => 1
        ]);
    }
    ...
    {
        $this->estimates_model->upgrade_level($id);
        $this->estimates_model->send_estimate_to_staff($id, '', true, '', true);
        $level = $this->levels_model->get_staff_level(get_staff_user_id());
        $this->approvedlogs_model->add([
            'estimateid' => $id,
            'staffid' => get_staff_user_id(),
