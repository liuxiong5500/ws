...
class Tickets extends Admin_controller
{
    ...
    public function add($userid = false)
    {
        ...
        } elseif ($this->input->get('opportunity_id') && $this->input->get('opportunity_id') > 0) {
            $data['opportunity_id'] = $this->input->get('opportunity_id');
            $data['ticket_type'] = 'opportunity';
            $data['userid'] = get_client_id_by_opportunity_id($data['opportunity_id']);
            if (total_rows('tblcontacts', ['active' => 1, 'userid' => $data['userid']]) == 1) {
                $contact = $this->clients_model->get_contacts($data['userid']);
                if (isset($contact[0])) {
                    $data['contact'] = $contact[0];
    ...
    public function ticket_change_data()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($ticket_type == 'opportunity') {
                $customer_has = customer_has_opportunities(get_user_id_by_contact_id($contact_id));
            } else {
                $customer_has = customer_has_projects(get_user_id_by_contact_id($contact_id));
            }
            echo json_encode([
                'contact_data' => $this->clients_model->get_contact($contact_id),
                'customer_has_opportunities' => $customer_has,
            ]);
        }
    }
