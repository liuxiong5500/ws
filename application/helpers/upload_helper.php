...
/**
 * Handles upload for opportunity files
 * @param  mixed $opportunity_id opportunity id
 * @return boolean
 */
function handle_opportunity_file_uploads($opportunity_id)
{
    $filesIDS = [];
    $errors = [];
    ...
    if (isset($_FILES['file']['name'])
        && ($_FILES['file']['name'] != '' || is_array($_FILES['file']['name']) && count($_FILES['file']['name']) > 0)) {
        do_action('before_upload_opportunity_attachment', $opportunity_id);
        ...
        $path = get_upload_path_by_type('opportunity') . $opportunity_id . '/';
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    $CI->db->insert('tblopportunityfiles', $data);
    ...
    if (count($filesIDS) > 0) {
        ...
        $lastFileID = key($filesIDS);
        $CI->opportunities_model->new_opportunity_file_notification($filesIDS[$lastFileID], $opportunity_id);
    }
...
function handle_supplier_contact_profile_image_upload($contact_id = '')
{
    if (isset($_FILES['profile_image']['name']) && $_FILES['profile_image']['name'] != '') {
        do_action('before_upload_supplier_contact_profile_image');
        if ($contact_id == '') {
            $contact_id = get_supplier_contact_user_id();
        }
        $path = get_upload_path_by_type('supplier_contact_profile_images') . $contact_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['profile_image']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->db->where('id', $contact_id);
                $CI->db->update('tblsuppliercontacts', [
                    'profile_image' => $filename,
                ]);
                // Remove original image
                unlink($newFilePath);
...
function get_upload_path_by_type($type)
{
    switch ($type) {
        ...
        case 'project':
            ...
            break;
        case 'opportunity':
            return OPPORTUNITY_ATTACHMENTS_FOLDER;
        ...
        case 'contract':
            ...
            break;
        case 'supplier_contact_profile_images':
            return SUPPLIER_CONTACT_PROFILE_IMAGES_FOLDER;
