 ...
 * @param  array $additional_data option to pass additional data for the templates eq is staff template or customer template
 * This field is also used for the opportunity discussion files and regular discussions
 * @return array
 */
function get_opportunity_merge_fields($opportunity_id, $additional_data = [])
{
    $fields = [];

    $fields['{opportunity_name}'] = '';
    $fields['{opportunity_deadline}'] = '';
    $fields['{opportunity_start_date}'] = '';
    $fields['{opportunity_description}'] = '';
    ...
    $fields['{opportunity_name}'] = $opportunity->name;
    $fields['{opportunity_deadline}'] = _d($opportunity->deadline);
    $fields['{opportunity_start_date}'] = _d($opportunity->start_date);
    $fields['{opportunity_description}'] = $opportunity->description;

    $custom_fields = get_custom_fields('opportunities');
    foreach ($custom_fields as $field) {
        $fields['{' . $field['slug'] . '}'] = get_custom_field_value($opportunity_id, $field['id'], 'opportunities');
    }
    ...
    if (isset($additional_data['discussion_id'])) {
        ...
        if (isset($additional_data['discussion_comment_id'])) {
            $CI->db->where('id', $additional_data['discussion_comment_id']);
            $discussion_comment = $CI->db->get('tblopportunitydiscussioncomments')->row();
            $fields['{discussion_comment}'] = $discussion_comment->content;
        }
    }
    if (isset($additional_data['customer_template'])) {
        $fields['{opportunity_link}'] = site_url('clients/opportunity/' . $opportunity_id);

        if (isset($additional_data['discussion_id']) && isset($additional_data['discussion_type']) && $additional_data['discussion_type'] == 'regular') {
            $fields['{discussion_link}'] = site_url('clients/opportunity/' . $opportunity_id . '?group=opportunity_discussions&discussion_id=' . $additional_data['discussion_id']);
        } elseif (isset($additional_data['discussion_id']) && isset($additional_data['discussion_type']) && $additional_data['discussion_type'] == 'file') {
            // is file
            $fields['{discussion_link}'] = site_url('clients/opportunity/' . $opportunity_id . '?group=opportunity_files&file_id=' . $additional_data['discussion_id']);
        }
    } else {
        $fields['{opportunity_link}'] = admin_url('opportunities/view/' . $opportunity_id);
        if (isset($additional_data['discussion_type']) && $additional_data['discussion_type'] == 'regular' && isset($additional_data['discussion_id'])) {
            $fields['{discussion_link}'] = admin_url('opportunities/view/' . $opportunity_id . '?group=opportunity_discussions&discussion_id=' . $additional_data['discussion_id']);
        } else {
            if (isset($additional_data['discussion_id'])) {
                // is file
                $fields['{discussion_link}'] = admin_url('opportunities/view/' . $opportunity_id . '?group=opportunity_files&file_id=' . $additional_data['discussion_id']);
            }
        }
    }
    ...
    $hook_data['merge_fields'] = $fields;
    $hook_data['fields_to'] = 'opportunity';
    $hook_data['id'] = $opportunity_id;
    $hook_data['additional_data'] = $additional_data;

    $hook_data = do_action('opportunity_merge_fields', $hook_data);
    $fields = $hook_data['merge_fields'];
...
function get_supplier_contact_merge_fields($client_id, $contact_id = '', $password = '')
{
    $fields = [];

    if ($contact_id == '') {
        $contact_id = get_primary_supplier_contact_user_id($client_id);
    }
    ...
    $client = $CI->supplier_model->get($client_id);

    if (!$client) {
        return $fields;
    }

    $CI->db->where('supplier_id', $client_id);
    $CI->db->where('id', $contact_id);
    ...
    if (!empty($client->vat)) {
        $fields['{supplier_vat_number}'] = $client->vat;
    }

    $fields['{supplier_company}'] = $client->company;
    $fields['{supplier_phonenumber}'] = $client->phonenumber;
    $fields['{supplier_country}'] = get_country_short_name($client->country);
    $fields['{supplier_city}'] = $client->city;
    $fields['{supplier_zip}'] = $client->zip;
    $fields['{supplier_state}'] = $client->state;
    $fields['{supplier_address}'] = $client->address;
    $fields['{supplier_id}'] = $client_id;
    ...
    $hook_data['merge_fields'] = $fields;
    $hook_data['fields_to'] = 'supplier_contact';
    $hook_data['id'] = $client_id;
    $hook_data['contact_id'] = $contact_id;

    $hook_data = do_action('supplier_contact_merge_fields', $hook_data);
    $fields = $hook_data['merge_fields'];
...
function get_estimate_merge_fields($estimate_id)
{
    ...
    $fields['{estimate_gp}'] = ($estimate->lcyprofit * 100) . '%';
