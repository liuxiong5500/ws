...
function get_relation_data($type, $rel_id = '', $connection_type = '', $connection_id = '')
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($rel_id != '') {
            ...
            $data = $CI->customer_warehouses_model->get($rel_id);
        } else {
            $where_warehouses = '';
            if ($CI->input->post('customer_id')) {
                $where_warehouses .= 'clientid=' . $CI->input->post('customer_id');
            }
            $search = $CI->misc_model->_search_warehouses($q, 0, $where_warehouses);
            $data = $search['result'];
        }
        ...
        if ($rel_id != '') {
            ...
            $data = $CI->opportunities_model->get($rel_id);
        } else {
            $where_opportunities = '';
            if ($CI->input->post('customer_id')) {
                $where_opportunities .= 'clientid=' . $CI->input->post('customer_id');
            }
            $search = $CI->misc_model->_search_opportunities($q);
            $data = $search['result'];
        }
    } elseif ($type == 'staff') {
        if ($rel_id != '') {
...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        $link = admin_url('opportunities/view/' . $id);
    } elseif ($type == 'warehouse') {
        if (is_array($relation)) {
            $id = $relation['id'];
            $name = $relation['name'];
