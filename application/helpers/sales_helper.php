...
{
    $CI = &get_instance();
    $CI->db->select(['tblitems_in.*','tblprocessmaster.name as process_name']);
    $CI->db->from('tblitems_in');
    $CI->db->where('rel_id', $id);
    $CI->db->where('rel_type', $type);
    $CI->db->join('tblitems', 'tblitems_in.item_id=tblitems.id');
    $CI->db->join('tblprocessmaster', 'tblitems.process_id=tblprocessmaster.id', 'left');
