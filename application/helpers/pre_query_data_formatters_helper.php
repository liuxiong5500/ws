...
add_action('before_purchase_order_updated', '_format_data_sales_feature');
add_action('before_purchase_order_updated', '_format_data_purchase_order_feature');
add_action('before_estimate_added', '_format_data_sales_feature');
add_action('before_purchase_order_added', '_format_data_sales_feature');
add_action('before_purchase_order_added', '_format_data_purchase_order_feature');
...
add_action('before_supplier_added', '_format_data_supplier');
add_action('before_supplier_updated', '_format_data_supplier');
add_action('before_create_supplier_contact', '_format_data_supplier');
add_action('before_update_supplier_contact', '_format_data_supplier');


function _format_data_purchase_order_feature($data)
{
    if (!isset($data['data']['discount_type'])) {
        $data['discount_type'] = 'before_tax';
    }
    return $data;
}
...
function _format_data_supplier($data)
{
    foreach (_get_supplier_unused_names() as $u) {
        if (isset($data['data'][$u])) {
            unset($data['data'][$u]);
        } elseif (isset($data[$u])) {
            unset($data[$u]);
...
function _get_supplier_unused_names()
{
    return [
        'fakeusernameremembered', 'fakepasswordremembered',
        'DataTables_Table_0_length', 'DataTables_Table_1_length',
