...
function task_rel_link($rel_id, $rel_type)
{
    ...
    } elseif ($rel_type == 'opportunity') {
        $link = admin_url('opportunities/view/' . $rel_id);
    }
...
function tasks_rel_name_select_query()
{
    return '(CASE rel_type
        ...
        WHEN "opportunity" THEN (SELECT CONCAT(CONCAT("#",tblopportunities.id)," - ",tblopportunities.name) FROM tblopportunities WHERE tblopportunities.id=tblstafftasks.rel_id)
...
function init_relation_tasks_table($table_attributes = [])
{
    ...
    if (has_permission('tasks', '', 'create')) {
        ...
        if ($table_attributes['data-new-rel-type'] != 'project' && $table_attributes['data-new-rel-type'] != 'opportunity') {
    ...
    } elseif ($table_attributes['data-new-rel-type'] == 'opportunity') {
        echo "<a href='" . admin_url('tasks/detailed_overview?opportunity_id=' . $table_attributes['data-new-rel-id']) . "' class='btn btn-success pull-right mbot25'>" . _l('detailed_overview') . '</a>';
        echo "<a href='" . admin_url('tasks/list_tasks?opportunity_id=' . $table_attributes['data-new-rel-id'] . '&kanban=true') . "' class='btn btn-default pull-right mbot25 mright5 hidden-xs'>" . _l('view_kanban') . '</a>';
        echo '<div class="clearfix"></div>';
        echo $CI->load->view('admin/tasks/_bulk_actions', ['table' => '.table-rel-tasks'], true);
        echo $CI->load->view('admin/tasks/_summary', ['rel_id' => $table_attributes['data-new-rel-id'], 'rel_type' => 'opportunity', 'table' => $table_name], true);
        echo '<a href="#" data-toggle="modal" data-target="#tasks_bulk_actions" class="hide bulk-actions-btn table-btn" data-table=".table-rel-tasks">' . _l('bulk_actions') . '</a>';
