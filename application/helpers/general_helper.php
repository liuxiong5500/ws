...
{
    if (!is_client_logged_in() && !is_staff_logged_in() && !is_supplier_logged_in()) {
        return false;
    }
...
function get_supplier_contact_user_id()
{
    $CI = &get_instance();
    if (!$CI->session->has_userdata('supplier_contact_user_id')) {
        return false;
    }

    return $CI->session->userdata('supplier_contact_user_id');
}
/**
 * Get timezones list
