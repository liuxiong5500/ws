...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="form-group select-placeholder opportunities-wrapper<?php if ((!isset($subscription)) || (isset($subscription) && !customer_has_opportunities($subscription->clientid))) {
            echo ' hide';
        } ?>">
            <label for="opportunity_id"><?php echo _l('opportunity'); ?></label>
            <div id="opportunity_ajax_search_wrapper">
                <select name="opportunity_id" id="opportunity_id" class="opportunities ajax-search ays-ignore"
                        data-live-search="true" data-width="100%"
                        data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                    <?php
                    if (isset($subscription) && $subscription->opportunity_id != 0) {
                        echo '<option value="' . $subscription->opportunity_id . '" selected>' . get_opportunity_name_by_id($subscription->opportunity_id) . '</option>';
                    }
                    ?>
                </select>
            </div>
