...
<script>
    ...
    $(function () {
        ...
        initDataTable('.table-customerwarehouses', admin_url + 'warehouses/table/' + customer_id, undefined, undefined, undefined, [0, 'desc']);
    });
    ...
    function validate_warehouse_form() {
        _validate_form('#warehouse-form', {
            name: 'required'
        }, warehouseFormHandler);
    }

    function warehouseFormHandler(form) {
        var formURL = $(form).attr("action");
        var formData = new FormData($(form)[0]);
        ...
        $.ajax({
            ...
            if ($.fn.DataTable.isDataTable('.table-customerwarehouses')) {
                $('.table-customerwarehouses').DataTable().ajax.reload(null, false);
            }
        }).fail(function (error) {
            alert_float('danger', JSON.parse(error.responseText));
        });
    ...
    function warehouse(client_id, warehouse_id) {
        if (typeof (warehouse_id) == 'undefined') {
            warehouse_id = '';
        }
        requestGet('clients/warehouse/' + client_id + '/' + warehouse_id).done(function (response) {
            $('#warehouse_data').html(response);
            $('#warehouse').modal({
                show: true,
                backdrop: 'static'
            });
            $('body').off('shown.bs.modal', '#warehouse');
            $('body').on('shown.bs.modal', '#warehouse', function () {
                if (warehouse_id == '') {
                    $('#warehouse').find('input[name="name"]').focus();
                }
            });
            validate_warehouse_form();
        }).fail(function (error) {
            var response = JSON.parse(error.responseText);
            alert_float('danger', response.message);
        });
