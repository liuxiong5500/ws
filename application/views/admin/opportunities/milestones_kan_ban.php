...
foreach ($milestones as $milestone) {
    ...
    $cpicker = '';
    if (has_permission('opportunities', '', 'create') && $milestone['id'] != 0) {
    ...
    $total_pages = ceil($this->opportunities_model->do_milestones_kanban_query($milestone['id'], $opportunity_id, 1, $milestonesTasksWhere, true) / get_option('tasks_kanban_limit'));

    $tasks = $this->opportunities_model->do_milestones_kanban_query($milestone['id'], $opportunity_id, 1, $milestonesTasksWhere);
    ...
    ?>
    <ul class="kan-ban-col milestone-column<?php if (!has_permission('opportunities', '', 'edit') || $milestone['id'] == 0) {
        ...
        <li class="kan-ban-col-wrapper">
            <div class="border-right panel_s">
                <div class="panel-heading panel-heading-bg <?php if ($milestone_color != '') {
                    ...
                    <?php if ($milestone['id'] != 0 && has_permission('opportunities', '', 'edit')) { ?>
                    ...
                    <a href="#"
                       ...
                       class="edit-milestone-phase <?php if ($milestone['color'] != '') {
                           echo 'color-white';
                       } ?>">
                        <?php } ?>
                ...
                <?php } ?>
                    ...
                    <?php if ($milestone['id'] != 0 && (has_permission('tasks', '', 'create') || has_permission('opportunities', '', 'create'))) { ?>
  ...
  <div class='text-center'><?php if (has_permission('tasks', '', 'create')) {
                            ?><button type='button' return false;' class='btn btn-success btn-block mtop10 new-opportunity-task-to-milestone'>
   ...
   <?php echo _l('reset_to_default_color'); ?>
                ...
                <div class="kan-ban-content-wrapper">
                    <div class="kan-ban-content">
                        <ul class="status opportunity-milestone milestone-tasks-wrapper sortable relative"
                            data-task-status-id="<?php echo $milestone['id']; ?>">
                            <?php
                            foreach ($tasks as $task) {
                                $this->load->view('admin/opportunities/_milestone_kanban_card', array('task' => $task, 'milestone' => $milestone['id']));
                            } ?>
                            <?php if ($total_tasks > 0) { ?>
                                <li class="text-center not-sortable kanban-load-more"
                                    ...
                                    <a href="#" class="btn btn-default btn-block<?php if ($total_pages <= 1) {
                                       ...
                                       onclick="kanban_load_more(<?php echo $milestone['id']; ?>,this,'opportunities/milestones_kanban_load_more',320,360); return false;"
                                       ;>
