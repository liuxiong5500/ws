<div id="setup-menu-wrapper"
     class="animated <?php if ($this->session->has_userdata('setup-menu-open') && $this->session->userdata('setup-menu-open') == true) {
         echo 'display-block';
     } ?>">
    <ul class="nav metis-menu" id="setup-menu">
        ...
        foreach ($menu_active->setup_menu_active as $item) {
            ...
            ?>
            <li>
                <a href="<?php echo $url; ?>"><?php if (!empty($item->icon)) { ?><i
                        class="<?php echo $item->icon; ?> menu-icon"></i><?php } ?><?php echo _l($item->name, '', false); ?>
                    <?php if ($submenu == true) { ?>
        ...
        <?php if (get_option('show_help_on_setup_menu') == 1 && is_admin()) {
            $total_setup_items++; ?>
            <li>
                <a href="<?php echo do_action('help_menu_item_link', 'https://www.visasoft.com.hk/vcrm/help'); ?>"
