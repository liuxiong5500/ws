...
$join = [
    ...
    'LEFT JOIN tblopportunities ON tblopportunities.id = tblprojects.opportunity_id',
];
...
if ($this->ci->input->get('opportunity_id') > 0) {
    array_push($where, 'AND tblprojects.opportunity_id=' . $this->ci->input->get('opportunity_id'));
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    ...
    'tblprojects.opportunity_id',
    'tblopportunities.name',
]);
...
foreach ($rResult as $aRow) {
    ...
    $name = '<a href="' . $link . '">' . $aRow['tblprojects.name'] . '</a>';
    ...
    $row[] = _d($aRow['tblprojects.start_date']);

    $row[] = _d($aRow['tblprojects.deadline']);
    ...
    $row[] = '<span class="label label inline-block project-status-' . $aRow['tblprojects.status'] . '" style="color:' . $status['color'] . ';border:1px solid ' . $status['color'] . '">' . $status['name'] . '</span>';
