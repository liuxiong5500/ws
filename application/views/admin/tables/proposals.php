...
if ($this->ci->input->post('expired')) {
    array_push($filter, 'OR open_till IS NOT NULL AND open_till <"' . date('Y-m-d') . '" AND tblproposals.status NOT IN(2,3)');
}

if ($this->ci->input->get('opportunity_id')) {
    array_push($where, 'AND tblproposals.opportunity_id=' . $this->ci->input->get('opportunity_id'));
}
...
if (count($statusIds) > 0) {
    array_push($filter, 'AND tblproposals.status IN (' . implode(', ', $statusIds) . ')');
}
...
array_push($join, 'LEFT JOIN tblopportunities ON tblopportunities.id = tblproposals.opportunity_id');
...
foreach ($rResult as $aRow) {
    ...
    $row[] = format_proposal_status($aRow['tblproposals.status']);
