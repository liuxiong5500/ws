...
$join = [
    ...
    'LEFT JOIN tblopportunities ON tblopportunities.id = tblexpenses.opportunity_id',
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'tblexpenses.id') {
            ...
            $_data = _d($_data);
        } elseif ($aColumns[$i] == 'opportunity_id') {
            if ($_data) {
                $_data = '<a href="' . admin_url('opportunities/view/' . $aRow['opportunity_id']) . '">' . $aRow['opportunity_name'] . '</a>';
            } else {
                $_data = '';
            }
