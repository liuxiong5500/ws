...
if ($timesheets_type == 'project') {
    $aColumns = do_action('projects_timesheets_table_sql_columns', $aColumns);
} elseif ($timesheets_type == 'opportunity') {
    $aColumns = do_action('opportunities_timesheets_table_sql_columns', $aColumns);
}
...
if ($timesheets_type == 'project') {
    ...
    $join = do_action('opportunities_timesheets_table_sql_join', $join);

    $where = ['AND task_id IN (SELECT id FROM tblstafftasks WHERE rel_id="' . $opportunity_id . '" AND rel_type="opportunity")'];
}

if (!(has_permission('projects', '', 'create') || has_permission('opportunities', '', 'create'))) {
...
if ($timesheets_type == 'project') {
    ...
    $staff_ids = $this->ci->opportunities_model->get_distinct_tasks_timesheets_staff($opportunity_id);
}
