...
class Estimates_model extends CRM_Model
{
    ...
    public function send_estimate_to_staff($id, $template = '', $attachpdf = true, $cc = '', $manually = false)
    {
        ...
        $estimate = $this->get($id);
        $template = 'estimate-approve-request';
        $estimate_number = format_estimate_number($estimate->id);
        ...
        if ($manually === true) {
            $sent_to = [];
            $staffs = $this->levels_model->get_staffs($estimate->currentlevel, $estimate->departmentid);
            foreach ($staffs as $staff) {
                array_push($sent_to, $staff['email']);
            }
        }
        ...
        if (is_array($sent_to) && count($sent_to) > 0) {
            ...
            foreach ($sent_to as $staff_mail) {
                if ($staff_mail != '') {
                    ...
                    if ($this->emails_model->send_email_template($template, $staff_mail, $merge_fields, '', $cc)) {
                        $sent = true;
                        array_push($emails_sent, $staff_mail);
                    }
                }
                $i++;
            }
    ...
    public function upgrade_level($id)
    {
        $estimate = $this->get($id);
        $this->db->where('id', $id);
        $this->db->update('tblestimates', ['currentlevel' => $estimate->currentlevel - 1]);
    }
