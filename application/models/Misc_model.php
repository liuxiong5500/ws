...
class Misc_model extends CRM_Model
{
    ...
    public function _search_opportunities($q, $limit = 0, $where = false)
    {
        $result = [
            'result' => [],
            'type' => 'opportunities',
            'search_heading' => _l('opportunities'),
        ];

        $opportunities = has_permission('opportunities', '', 'view');
        // Projects
        $this->db->select();
        $this->db->from('tblopportunities');
        if (!$opportunities) {
            $this->db->where('tblopportunities.id IN (SELECT opportunity_id FROM tblopportunitymembers WHERE staff_id=' . get_staff_user_id() . ')');
        }
        if ($where != false) {
            $this->db->where($where);
        }
        if (!_startsWith($q, '#')) {
            $this->db->where('(description LIKE "%' . $q . '%"
                OR name LIKE "%' . $q . '%"
                )');
        } else {
            $this->db->where('id IN
                (SELECT rel_id FROM tbltags_in WHERE tag_id IN
                (SELECT id FROM tbltags WHERE name="' . strafter($q, '#') . '")
                AND tbltags_in.rel_type=\'opportunity\' GROUP BY rel_id HAVING COUNT(tag_id) = 1)
                ');
        }
    ...
    public function _search_warehouses($q, $limit = 0, $where = false)
    {
        $result = [
            'result' => [],
            'type' => 'warehouses',
            'search_heading' => _l('warehouses'),
        ];
        $this->db->select();
        $this->db->from('tblcustomerwarehouses');
        if ($where != false) {
            $this->db->where($where);
        }
        ...
        if (!_startsWith($q, '#')) {
            $this->db->where('(
                tblcustomerwarehouses.name LIKE "' . $q . '"
                OR
                tblcustomerwarehouses.address LIKE "%' . $q . '%"
                OR
                tblcustomerwarehouses.contact LIKE "%' . $q . '%"
                OR
                tblcustomerwarehouses.telephone LIKE "%' . $q . '%"
                )');
        }
