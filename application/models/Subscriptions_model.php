...
class Subscriptions_model extends CRM_Model
{
    ...
    {
        $this->db->select('tblsubscriptions.id as id, date, next_billing_cycle, status, tblsubscriptions.project_id as project_id,tblsubscriptions.opportunity_id as opportunity_id, description, tblsubscriptions.created_from as created_from, tblsubscriptions.name as name, tblcurrencies.name as currency_name, tblcurrencies.symbol, currency, clientid, ends_at, date_subscribed, stripe_plan_id,stripe_subscription_id,quantity,hash,tbltaxes.name as tax_name, tbltaxes.taxrate as tax_percent, tax_id, stripe_id as stripe_customer_id,' . get_sql_select_client_company());
    }
