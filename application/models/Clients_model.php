...
class Clients_model extends CRM_Model
{
    ...
    public function get_warehouses($customer_id = '')
    {
        if ($customer_id != '') {
            $this->db->where('userid', $customer_id);
        }
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('tblcustomerwarehouses', $data);
        if ($this->db->affected_rows() > 0) {
            logActivity('Warehouse Updated [' . $id . ']');
    ...
    public function add_warehouse($data, $customer_id)
    {
        unset($data['id']);
        $this->db->insert('tblcustomerwarehouses', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            logActivity('New Warehouse Added [ID: ' . $insert_id . ']');
            return true;
        }
        ...
        if ($contact_id) {
            ...
            foreach ($permissions as $permission) {
                ...
                if ($not_manual_request == true) {
                    ...
                    } elseif ($permission == 24) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblcontacts', ['opportunity_emails' => 1, 'task_emails' => 1]);
                    }
                }
            }
